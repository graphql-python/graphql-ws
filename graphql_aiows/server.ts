          ...
          connectionContext.initPromise.then(initResult => {
            ...
            promisedParams
              .then((params: any) => {
                ...
                let executionIterable: Promise<
                  AsyncIterator<ExecutionResult> | ExecutionResult
                >;
                const validationErrors: Error[] = validate(
                  this.schema,
                  document,
                  this.specifiedRules
                );
                if (validationErrors.length > 0) {
                  executionIterable = Promise.resolve(
                    createIterableFromPromise<ExecutionResult>(
                  ...
                  if (
                    ...
                    executionIterable = Promise.resolve(
                      (promiseOrIterable as any) as AsyncIterator<
                        ExecutionResult
                      >
                    );
                  } else {
